<template>
  ...
  <div class="p-4 sm:p-8 max-w-7xl mx-auto bg-white shadow-md rounded-xl">
    <h2 class="text-2xl sm:text-3xl font-semibold text-center text-gray-800 mb-6">Vendor Management</h2>

    <!-- Toggle Form Section -->
    <div class="flex justify-between items-center mb-4 sm:mb-6">
      <h3 class="text-lg font-medium">Vendor Form</h3>
    ...
    <div v-show="showForm" class="flex flex-wrap gap-4 justify-start sm:justify-center mb-6">
      <input v-model="vendor.name" placeholder="Name" class="input-style w-full sm:w-auto" />
      <input v-model="vendor.email" placeholder="Email" class="input-style w-full sm:w-auto" />
      <input v-model="vendor.phone" placeholder="Phone" class="input-style w-full sm:w-auto" />
      <input v-model="vendor.address" placeholder="Address" class="input-style w-full sm:w-auto" />
      <div class="flex gap-2">
        <button @click="saveVendor" class="btn-primary">
          {{ vendor.id ? 'Update' : 'Save' }}
        </button>
        <button @click="resetForm" class="btn-secondary">Reset</button>
      </div>
      <input v-model="search" @input="searchVendors" placeholder="Search by any field..." class="input-style w-full" />
    </div>

    <!-- Toggle Table Section -->
    <div class="flex justify-between items-center mb-4">
      <h3 class="text-lg font-medium">Vendor List</h3>
    ...
    <div v-show="showTable" class="overflow-auto rounded-lg shadow">
      <table class="min-w-full text-sm text-left text-gray-700">
        ...
        <tbody>
          <tr v-for="(c, index) in paginatedVendors" :key="c.id" class="border-b">
            ...
            <td class="px-6 py-2 space-x-2">
              <button @click="updateVendor(c)" class="btn-primary-sm">Update</button>
              <button @click="deleteVendor(c.id)" class="btn-danger-sm">Delete</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <p class="mt-4 text-sm text-center text-gray-500">
      Showing {{ paginatedVendors.length }} of {{ vendors.length }} vendors (5 per page)
    </p>
  </div>
</template>
...
const vendors = ref([])
const vendor = ref({ name: '', email: '', phone: '', address: '' })
const search = ref('')
...
const paginatedVendors = computed(() => {
  ...
  return vendors.value.slice(start, start + itemsPerPage)
})
...
const saveVendor = async () => {
  try {
    if (vendor.value.id) {
      await VendorService.update(vendor.value.id, vendor.value)
    } else {
      await VendorService.create(vendor.value)
    }
    resetForm()
    loadVendors()
  } catch (err) {
...
const updateVendor = async (vendor) => {
  if (!vendor.id) return alert("Vendor ID missing.")
  try {
    await VendorService.update(vendor.id, vendor)
...
const resetForm = () => {
  vendor.value = { name: '', email: '', phone: '', address: '' }
}

const searchVendors = async () => {
  try {
    if (!search.value.trim()) return loadVendors()
    const res = await VendorService.search(search.value)
    vendors.value = res.data
    currentPage.value = 1
  } catch (err) {
...
const exportToExcel = () => {
  const data = vendors.value.map(c => [c.name, c.email, c.phone, c.address])
...
const exportToPDF = () => {
  ...
  autoTable(doc, {
    ...
    body: vendors.value.map(c => [c.name, c.email, c.phone, c.address]),
  })
